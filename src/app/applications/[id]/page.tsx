...
export default function ApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const updateStatus = async (newStatus: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setApplication(prev => prev ? { ...prev, status: apiStatus } : null)
  ...
  const getBasicInfoFields = (): BasicInfoItem[] => {
    ...
    application.formData.forEach(field => {
      ...
      if (!experienceAdded) {
        if (label.includes('years of') && label.includes('experience')) {
        ...
        } else if (label.includes('professional') && label.includes('experience')) {
          basicFields.push({
            label: 'Professional Experience',
            value: field.value
          })
          experienceAdded = true
        } else if (label.includes('work experience') || label.includes('working experience')) {
          basicFields.push({
          ...
          experienceAdded = true
        } else if (label.includes('experience') && !label.includes('describe') && !label.includes('explain') && !label.includes('team') && !label.includes('leading')) {
          basicFields.push({
            label: 'Experience',
            value: field.value
          })
      ...
      else if (label.includes('current position') || label.includes('current role')) {
        basicFields.push({
      ...
      // Education fields
      else if (label.includes('education') || label.includes('qualification') || label.includes('degree')) {
        basicFields.push({
          label: 'Education',
          value: field.value
        })
      ...
      // Salary fields
      else if (label.includes('current salary') || label.includes('present salary')) {
        basicFields.push({
          label: 'Current Salary',
          value: field.value
        })
      } else if (label.includes('expected salary') || label.includes('salary expectation')) {
        basicFields.push({
      ...
      // Location fields - only add from form data if it exists and is not localhost
        ...
        if (typeof field.value === 'string' && 
            ...
            !field.value.includes('@')) { // Exclude email addresses
          basicFields.push({
            label: 'Location',
            value: field.value
          })
  ...
  const getContactAndMetadata = (): BasicInfoItem[] => {
    ...
    if (application.candidateCity || application.candidateState || application.candidateCountry || application.candidateIP) {
      ...
      if (application.candidateCity || application.candidateState || application.candidateCountry) {
        const locationParts = []
        if (application.candidateCity) locationParts.push(application.candidateCity)
        if (application.candidateState) locationParts.push(application.candidateState)
        if (application.candidateCountry) locationParts.push(application.candidateCountry)
        
        if (locationParts.length > 0) {
          contactFields.push({
            label: isLocalhostData ? 'Location (Dev Environment)' : 'Location (IP-based)',
      ...
      if (application.candidateIP) {
        contactFields.push({
          label: 'IP Address',
          value: isLocalhostData ? `${application.candidateIP} (localhost)` : application.candidateIP
        })
      }
      
      // Show coordinates if available (production only)
      if (application.candidateLatitude && application.candidateLongitude && !isLocalhostData) {
        contactFields.push({
          label: 'Coordinates',
    ...
    application.formData.forEach(field => {
      ...
      if (!userLocationFound && 
          (label.includes('location') || label.includes('city') || 
           label.includes('state') || label.includes('country') ||
           (label.includes('address') && !label.includes('email'))) &&
          !label.includes('email') && !label.includes('portfolio') &&
    ...
    application.formData.forEach(field => {
        ...
        if (Array.isArray(field.value) && field.value.length > 0) {
          field.value.forEach((link, index) => {
            if (typeof link === 'string' && link.trim() !== '') {
              const linkLabel = link.includes('github') ? 'GitHub' : 
      ...
      else if (label.includes('preferred contact') || label.includes('contact preference')) {
        contactFields.push({
  ...
  const getQuestionsAndAnswers = (): QAItem[] => {
    ...
    application.formData.forEach(field => {
      ...
      if (label.includes('name') && (label.includes('full') || label.includes('first') || label.includes('last')) ||
      ...
      if (typeof field.value === 'string') {
        ...
        if (field.fieldType === 'SKILLS' || 
            (field.value.startsWith('[') && field.value.includes('skill') && field.value.includes('rating')) ||
        ...
        if (field.value.length > 0 && field.fieldType !== 'SKILLS' && !label.includes('skill')) {
          qaFields.push({
            question: field.label,
  ...
  const renderSkillRating = (skillData: string | { skill: string; rating: number }) => {
    // If it's a skill with rating data
    if (typeof skillData === 'object' && skillData.skill && skillData.rating) {
      return (
        <div className="flex items-center space-x-2 py-1">
          <span className="text-sm font-medium text-gray-900 min-w-0 flex-1">{skillData.skill}</span>
          <div className="flex">
            {[1, 2, 3, 4, 5].map((star) => (
              <Star
                key={star}
          ...
          <span className="text-xs text-gray-500 whitespace-nowrap">({skillData.rating}/5)</span>
        </div>
      )
    }
    ...
    if (typeof skillData === 'string') {
      ...
      if (skillData.startsWith('{') || skillData.startsWith('[')) {
        try {
          ...
          } else if (Array.isArray(parsed) && parsed.length > 0 && parsed[0].skill) {
    ...
    return (
      <div className="flex items-center space-x-2 py-1">
        <span className="text-sm font-medium text-gray-900 min-w-0 flex-1">{skillName}</span>
        {rating > 0 ? (
          <>
            <div className="flex">
              {[1, 2, 3, 4, 5].map((star) => (
            ...
            <span className="text-xs text-gray-500 whitespace-nowrap">({rating}/5)</span>
          </>
        ) : (
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/applications" className="text-indigo-600 hover:text-indigo-500">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-4">
            ...
            <nav className="flex space-x-8">
              ...
              <Link href="/applications" className="text-gray-700 hover:text-gray-900">Job Applications</Link>
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-800">
                Apply for: <span className="font-semibold">{application.jobTitle}</span> | {application.company}
              </p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-2">
                {skillsFields.length > 0 ? (
                  skillsFields.map((field, index) => {
                    ...
                    if (field.fieldType === 'SKILLS' && typeof field.value === 'string') {
                      try {
                        const skillRatings = JSON.parse(field.value)
                        if (Array.isArray(skillRatings)) {
                          return skillRatings.map((skillRating, skillIndex) => (
                    ...
                    // Handle array of skills (legacy support)
                      ...
                      return field.value.map((skill: string, skillIndex: number) => (
                        <div key={`${index}-${skillIndex}`}>
                  ...
                  <div className="text-center py-8">
                    <p className="text-gray-500 text-sm">No skills information provided in this application.</p>
                    <div className="mt-2 text-xs text-gray-400">
                      Debug: Found {application?.formData?.length || 0} total form fields
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg p-6 shadow-sm border">
              <h3 className="text-lg font-semibold text-gray-900 mb-6">Questions and Answers</h3>
              <div className="space-y-6">
                {questionsAndAnswers.length > 0 ? (
                  questionsAndAnswers.map((qa, index) => (
                    <div key={index} className="border-b border-gray-100 pb-6 last:border-b-0 last:pb-0">
                      <h4 className="font-semibold text-gray-900 mb-3 text-base">{qa.question}</h4>
                  ...
                  <div className="text-center py-8">
                    <p className="text-gray-500 text-sm">No additional questions were answered in this application.</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
            <div className="p-6">
              ...
              <p className="text-sm text-gray-500 text-center mb-6">
                Do you really want to reject this application? This action will notify the candidate and cannot be easily undone.
              </p>
              <div className="flex space-x-3">
                <button
